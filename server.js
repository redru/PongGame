...
app.post('/register', function Register(req, res) {
    ...
    if (!Match.isConnectedUser(username)) {
        return Promise.resolve(Match.register(username)).then(function(user) {
            return Authentication.generateToken({id: user.id, username: user.username}).then(function(err, token) {
                return err ? Promise.reject(err) : Promise.resolve(token);
            });
        }).then(function(data) {
            console.log('Connected user', data.user.username);

            res.header('JWT', data.token);
            return res.status(200).json({ data: { status: 0, result: data.user.username } });
        }).catch(function(reason) {
            console.log(reason);
            return res.status(200).json({ data: { status: 1, result: null } });
        });
    } else {
        return res.status(200).json({ data: { status: 1, result: null } });
    }
